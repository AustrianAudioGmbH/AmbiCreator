    ...
    AudioProcessorEditor (&p), processor (p), valueTreeState (vts)
{
    ...
    cbAttOutChOrder.reset (new ComboBoxAttachment (valueTreeState,
                                                   "channelOrder",
                                                   *title.getOutputWidgetPtr()->getCbOutChOrder()));
    ...
    slAttOutGain.reset (
        new ReverseSlider::SliderAttachment (valueTreeState, "outGainDb", slOutGain));
    ...
    slAttHorizontalRotation.reset (
        new ReverseSlider::SliderAttachment (valueTreeState, "horRotation", slHorizontalRotation));
    ...
    tbLegacyMode.setToggleState (processor.isLegacyModeActive(),
...
void AmbiCreatorAudioProcessorEditor::paint (Graphics& g)
{
    ...
    if (processor.isLegacyModeActive())
    {
        g.drawImage (arrayLegacyImage,
                     ...
                     0,
                     0,
                     arrayLegacyImage.getWidth(),
                     arrayLegacyImage.getHeight());
        helpToolTip.setTooltip (helpTextLegacy);
    }
    else
    {
        g.drawImageWithin (arrayImage,
                           12,
                           1,
                           arrayImage.getWidth() / 2,
                           arrayImage.getHeight() / 2,
...
void AmbiCreatorAudioProcessorEditor::resized()
{
    ...
    Rectangle<int> sliderArea = area.removeFromLeft (linearSliderWidth)
                                    .withHeight (3 * linearSliderVerticalSpacing + 3 * labelHeight
                                                 + 3 * linearSliderHeight);
    ...
    lbOutConfig.setBounds (outMeterArea.getX() - 7 * meterSpacing,
                           lbSlRotOutGain.getY(),
                           4 * meterWidth + 14 * meterSpacing,
                           labelHeight);
    cbOutChannelOrder.setBounds (outMeterArea.getX() - 7 * meterSpacing,
                                 outMeterArea.getY() - 2 * comboBoxHeight,
                                 4 * meterWidth + 14 * meterSpacing,
                                 comboBoxHeight);
